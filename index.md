...
<mark style="background-color:yellow;text-align:center">**Prenez 10 minutes à lire ce introduction**</mark>
...
Pour ce cours pratique, <mark style="background-color:gray;color:white">**nous allons travailler exclusivement en utilisant la ligne de commande**</mark>. De nos jours, il existe de nombreux environnements visuels pour travailler avec git, ainsi que des environnements intégrés dans différents IDE. Oubliez-les. Une fois que vous aurez appris à utiliser git sur la ligne de commande, vous n'aurez aucun problème à utiliser un environnement graphique. A l'inverse, ce n'est pas si facile.

Plus précisément, nous aurons besoin de : 

>1. Un éditeur de texte.
2. Une installation git :
2.1. Vous pouvez télécharger git [ici pour différentes plates-formes](https://git-scm.com/downloads)
2.1. Installation de git sur ubuntu : `sudo apt-get update; sudo apt-get install git `
2.2. [Installation de git sur windows 10](https://www.develves.net/blogs/asd/articles/using-git-with-powershell-on-windows-10/#installing-git)
